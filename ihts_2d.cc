...
#include "PipeSystem.h"
        ...
        soil_heat_fluxes(1, std::vector<double>(number_of_surface_heat_and_mass_fluxes))
  {
    ...
    {
      time_step = 900;
      timestep_number_max = 18044; // 18044;
      initial_date.reserve(6);
    ...
    {
      time_step = 900;
      timestep_number_max = 11516; // 11516;
      initial_date.reserve(6);
      initial_date.push_back(1);
      initial_date.push_back(11);
      initial_date.push_back(2006);
  ...
  void Heat_Pipe<dim>::refine_grid()
  {
    ...
    for (; cell != endc; ++cell)
    {
      ...
      if ((x < 14.8 && x > -7.8196 && y > -0.2) ||
          ...
          (x < -4. && x > -7.8196 && y > m * (x + 7.8196) - 2.6737 - 0.2))
                  ...
                  if (q_face_point == 0)
                  {
                    if (face_boundary_indicator == boundary_id_road)
                    {
                      road_heat_fluxes[timestep_number - 1][0] += // J/m
                          ...
                          ((theta)*new_boundary_flux.inbound_heat_flux_solar() +
                           (1. - theta) * old_boundary_flux.inbound_heat_flux_solar());
                      road_heat_fluxes[timestep_number - 1][1] += // J/m
                          time_step * cell_face_diameter *
                          ((theta)*new_boundary_flux.inbound_heat_flux_convective() +
                           (1. - theta) * old_boundary_flux.inbound_heat_flux_convective());
                      road_heat_fluxes[timestep_number - 1][2] += // J/m
                          time_step * cell_face_diameter *
                          ((theta)*new_boundary_flux.inbound_heat_flux_evaporative() +
                           (1. - theta) * old_boundary_flux.inbound_heat_flux_evaporative());
                      road_heat_fluxes[timestep_number - 1][3] += // J/m
                          time_step * cell_face_diameter *
                          ((theta)*new_boundary_flux.inbound_heat_flux_infrared() +
                      ...
                      road_heat_fluxes[timestep_number - 1][8] += // J/m
                          time_step * cell_face_diameter *
                          ((theta)*outbound_convective_coefficient_new *
                               new_surface_temperature +
                           (1. - theta) *
                               outbound_convective_coefficient_old *
                               old_surface_temperature);
                    ...
                    else
                    {
                      ...
                      soil_heat_fluxes[timestep_number - 1][3] += // J/m
                          time_step * cell_face_diameter *
                          ((theta)*new_boundary_flux.inbound_heat_flux_infrared() +
                           (1. - theta) * old_boundary_flux.inbound_heat_flux_infrared());
  ...
  void Heat_Pipe<dim>::output_results()
  {
     ...
     * Define output name
     */
    std::string filename = output_path + "/solution-" + preheating_output_filename + "-" + Utilities::int_to_string(timestep_number, 4) + "." + Utilities::int_to_string(this_mpi_process, 3) + ".vtu";
    ...
    if (this_mpi_process == 0)
    {
      ...
      for (unsigned int i = 0; i < n_mpi_processes; ++i)
        filenames.push_back("solution-" + preheating_output_filename + "-" + Utilities::int_to_string(timestep_number, 4) + "." + Utilities::int_to_string(i, 3) + ".vtu");

      const std::string
          pvtu_master_filename = ("output/solution-" + preheating_output_filename + "-" + Utilities::int_to_string(timestep_number, 4) + ".pvtu");
  ...
  void Heat_Pipe<dim>::surface_temperatures()
  {
     ...
     */
    {
      ...
      {
        unsigned int face = cell_index_to_face_index[it->first];
        unsigned int boundary_id = it->first->face(face)->boundary_id();

        if (boundary_id == boundary_id_soil)
        {
          current_new_avg_soil_surface_temperature += it->second;
        }
        if (boundary_id == boundary_id_road)
        {
          current_new_avg_road_surface_temperature += it->second;
        }
        if ((boundary_id == boundary_id_collector) ||
            (boundary_id == boundary_id_storage))
        {
          ...
          if (it->second > max_pipe_temperature[pipe_number])
            max_pipe_temperature[pipe_number] = it->second;
          if (it->second < min_pipe_temperature[pipe_number])
            min_pipe_temperature[pipe_number] = it->second;
  ...
  {
    if (date_and_time.size() == 0 && met_data.size() == 0)
    {
      if (analytic_met_data)
      {
        ...
        data_tools.seconds_to_date(date_and_time,all_dates_in_seconds);
        ...
        for (unsigned int i = 0; i < date_and_time.size(); i++)
        {
          ...
          met_data_line.push_back(analytic_solution.get_analytic_air_temperature(all_dates_in_seconds[i][0]));
          met_data_line.push_back(analytic_solution.get_analytic_relative_humidity(/*initial_time*/));
          met_data_line.push_back(analytic_solution.get_analytic_wind_speed(/*initial_time*/));
          met_data_line.push_back(/*wind direction*/ 0.);
          met_data_line.push_back(analytic_solution.get_analytic_solar_radiation(all_dates_in_seconds[i][0]));
          met_data_line.push_back(analytic_solution.get_analytic_precipitation(/*initial_time*/));
  ...
  void Heat_Pipe<dim>::run()
  {
         ...
         timestep_number++, time += time_step)
    {
      ...
      do
      {
        ...
        double avg_storage___outlets = 0.;
        {
          ...
          for (unsigned int i = 0; i < number_of_pipes / 2; i++)
          {
            ...
            current__new_storage___avg_norm += new_avg_pipe_temperature[(number_of_pipes / 2) + i] / (number_of_pipes / 2);
            ...
            if (i < number_of_pipes / 4)
            {
              ...
              avg_storage___outlets += max_pipe_temperature[(number_of_pipes / 4) + (number_of_pipes / 2) + i] / (number_of_pipes / 4);
            }
          }
          /*
           * Estimate errors
          ...
          tolerance_collector_avg_norm = fabs(1. - current__new_collector_avg_norm / previous_new_collector_avg_norm);
          tolerance_storage___avg_norm = fabs(1. - current__new_storage___avg_norm / previous_new_storage___avg_norm);
        }
        //==========================================================================
        /*-----solve pipe system ------*/
        if (step == 0)
        {
          ...
          else if (preheating_step == 5 || preheating_step == 8)
          {
             ...
             * is relatively small).
             */
            auto located_cell = GridTools::find_active_cell_around_point(triangulation, Point<dim>(0., -0.025));
          ...
          control_temperatures[timestep_number - 1][0] = new_control_temperature_collector;
          control_temperatures[timestep_number - 1][1] = new_control_temperature___storage;
          /*
           * First get the value of the swtich controlling the system ON and OFF status.
           * Two options: Automatic and Forced.
           * 
           * - Automatic:
           *   The system is activated automatically if a certain criteria is meet.
           *   The criteria is different depending on if the system is 'charging' (heating
           *   up the storage area during late-spring, summer, early-autumn) or 'usage'
           *   (heating up the pavement during winter).
           *   -- 'charging'
           ...
           *     active.
           *     Two main periods of time are identified in the experimental results:
           *       from 23/08/2005 to 14/11/2005 (preheating 4)
           *       
           *   -- 'usage':
           *     The system is activated whenever the pavement surface drops below 4C.
           *     Two main periods of time are identified in the experimental results:
           *       from from 15/11/2005 to 20/02/2006 (preheating 5)
           * 
           * - Forced:
           *   The system is forced to be active in certain ranges of time independently
           *   of soil and weather conditions.(i.e. between noon and 10pm everyday).
           * 
           *  NOTE: The experimental results showed a couple of periods when the system stopped
           *  due to failure. This is hardcoded in the following block. The periods are:
           *    from 23/08/2005 00:00:00 to 23/08/2005 13:00:00 (in preheating 4)
           *    from 18/05/2005 00:00:00 to 05/06/2005 00:00:00 (in preheating 7)
           */
          if (activation_type == "automatic_activation")
          {
            ...
            { // second period: from 15/11/2005 to 20/02/2006
              ...
              if (new_control_temperature___storage < new_control_temperature_collector)
                switch_control = false;
            }
            else
              switch_control = false;
        ...
        if ((pipe_system == true) && (switch_control == true))
        {
          if ((previous_switch_control == false) && (switch_control == true) && (step == 0))
          {
            for (unsigned int i = 0; i < number_of_pipes / 2; i++)
            {
              current__new_inlet__temperatures_pipes[i]      = new_avg_pipe_temperature[20 + i];
              current__new_inlet__temperatures_pipes[20 + i] = new_avg_pipe_temperature[i];
            ...
            current__new_inlet__temperatures_pipes = previous_new_inlet__temperatures_pipes;
          ...
          for (unsigned int i = 0; i < number_of_pipes; i++)
            current__new_pipe_heat_flux[i] += current_heat_flux_temp[i];

          previous_new_inlet__temperatures_pipes = current__new_inlet__temperatures_pipes;
        }
        /*
        ...
        cell_index_to_new_previous_surface_temperature = cell_index_to_new_current__surface_temperature;
        ...
        if (timestep_number == timestep_number_max)
        {
          ...
          if (this_mpi_process == 0)
          {
            std::vector<std::vector<int>>::const_iterator
                first = date_and_time.begin(),
                second = date_and_time.begin() + timestep_number_max;
      ...
      soil_heat_fluxes.resize(soil_heat_fluxes.size() + 1, std::vector<double>(number_of_surface_heat_and_mass_fluxes));
      {
        ...
        cell_index_to_old_surface_temperature = cell_index_to_new_current__surface_temperature;
        old_avg_soil_surface_temperature = current_new_avg_soil_surface_temperature;
        old_avg_road_surface_temperature = current_new_avg_road_surface_temperature;
        old_avg_pipe_temperature = new_avg_pipe_temperature;
        old_inlet__temperatures_pipes = current__new_inlet__temperatures_pipes;
        old_outlet_temperatures_pipes = current__new_outlet_temperatures_pipes;
        old_pipe_heat_flux = current__new_pipe_heat_flux;
        previous_switch_control = switch_control;
        old_control_temperature_collector = new_control_temperature_collector;
      }
    }
    /*
